...
namespace MonCineTests
{
    public class FilmTests
    {
        ...
        private List<Film> GenererFilms()
        {
            ...
            int startIndex = FilmTests._rand.Next(0, _acteurs.Count);
            List<ObjectId> acteursId = new List<ObjectId>();
            _acteurs
                .GetRange(startIndex, FilmTests._rand.Next(0, _acteurs.Count - startIndex))
                .ForEach(x => acteursId.Add(x.Id));
            startIndex = FilmTests._rand.Next(0, _realisateurs.Count);
            List<ObjectId> realisateursId = new List<ObjectId>();
            _realisateurs
                .GetRange(startIndex, FilmTests._rand.Next(0, _realisateurs.Count - startIndex))
            ...
            foreach (string nom in nomsFilm)
            {
                ...
                (
                    ...
                    new List<Projection>(),
                    GenererNotes(),
                    categoriesId[FilmTests._rand.Next(0, categoriesId.Count - 1)],
                    acteursId,
                    realisateursId
                );
                films.Add(film);
        ...
        {
            int nbNotes = FilmTests._rand.Next(1, 11);
            List<int> indexAbonnesOntNote = new List<int>();
            List<Note> notes = new List<Note>();
            for (int i = 0; i < nbNotes; i++)
            {
                int index = FilmTests._rand.Next(0, _abonnes.Count - 1);
                if (!indexAbonnesOntNote.Contains(index))
                {
                    notes.Add(new Note(_abonnes[index].Id, FilmTests._rand.Next(10)));
        ...
        {
            // Act et Assert
            Assert.Equal(_films, _dalFilm.ObtenirTout());
        }

        [Fact]
        public void ObtenirUnRetourneFilmSelonFiltre()
        {

            // Act et Assert
            Assert.Equal(_films[0], _dalFilm.ObtenirUn(_films[0].Id));
        }
        ...
        {
            // Act et Assert
            Assert.Equal(new List<Film> { _films[0], _films[2] },
                _dalFilm.ObtenirPlusieurs(new List<ObjectId> { _films[0].Id, _films[2].Id }));
        }
        ...
        {
            // Act et Assert
            Assert.True(_dalFilm.InsererPlusieurs(new List<Film>
            {
                new Film(ObjectId.GenerateNewId(), "A", DateTime.Now, new List<Projection>(), new List<Note>(),
                    _categories[0].Id, new List<ObjectId> { _acteurs[0].Id },
                    new List<ObjectId> { _realisateurs[0].Id })
            }));
        }
